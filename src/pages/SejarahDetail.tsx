...
const SejarahDetail = () => {
  const { id } = useParams();
  const item = sejarahData.find(item => item.id === id);
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div className="container mx-auto px-4 py-20 text-center">
          <h1 className="text-4xl font-bold mb-4">Sejarah tidak ditemukan</h1>
          <Button asChild>
            <Link to="/sejarah">Kembali ke Daftar Sejarah</Link>
          </Button>
        </div>
        <Footer />
      </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <Button asChild variant="outline" className="mb-8">
            <Link to="/sejarah" className="flex items-center space-x-2">
              <ArrowLeft size={16} />
